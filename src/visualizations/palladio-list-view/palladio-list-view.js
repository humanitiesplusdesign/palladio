...
angular.module('palladioListView', [])
	.constant('version', '0.9')
	.directive('palladioListView', function () {
		...
		var directiveDefObj = {
			...
			link: function (scope, element, attrs) {
				...
				scope.$on('update', function(event) {
					...
					if(element.is(':visible')) { updateList(); }
				});

				// Update when it becomes visible (updating when not visibile errors out)
				scope.$watch(function() { return element.is(':visible'); }, updateList);
	...
	.directive('palladioListViewWithSettings', function () {
		...
		return {
			...
			template :	'<!-- View -->' +
						...
						'<div class="span4 settings open"> ' +
							'<p class="super tiny">Gallery Settings</p>' +
							'<div class="row-fluid">' +
			...
			link: {
				...
				pre: function (scope, element, attrs) {
					...
					scope.$emit('registerStateFunctions', ['listView', exportState, importState]);
