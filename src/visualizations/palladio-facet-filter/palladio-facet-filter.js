...
angular.module('palladioFacetFilter', [])
	.constant('version', '0.9')
	.directive('palladioFacetFilter', function () {
		...
		var directiveDefObj = {
			...
			link: function (scope, element, attrs) {
				...
				function addDimension(dim) {
					...
					var chart = elastic_list().dimension(dimension).callback(
						function() {
							if(listMap.get(domID).filter()) {
								scope.$emit('updateFilter', [dim.config.identifier, dim.config.description, listMap.get(domID).filter(), listMap.get(domID).resetFilter]);
							} else {
								scope.$emit('updateFilter', [dim.config.identifier, null]);
							}
					...
					deregister.push(scope.$on('update', function() {
						actions.update();
					}));

					deregister.push(scope.$on('filterReset', function() {
						actions.resetFilter();
					}));
	...
	.directive('palladioFacetFilterWithSettings', function () {
		return {
			scope : true,
			...
			link : {
				pre : function(scope, element, attrs) {
					...
					// reasons.
					scope.fields = scope.metadata.filter(function (d) {
						return ( d.type === 'text' || d.type === 'date' ) && !d.uniqueKey && d.cardinality < 1000;
					}).sort(function (a, b) { return a.description < b.description ? -1 : 1; });

					// Get the countable fields.
					scope.countFields = scope.metadata.filter(function (d) {
					...
					scope.$emit('registerStateFunctions', ['facet', exportState, importState]);
				},
