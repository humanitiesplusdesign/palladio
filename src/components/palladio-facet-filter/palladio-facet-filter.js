		...
		var directiveDefObj = {
			...
			link: function (scope, element, attrs) {
				...
				scope.resetFilter = function () {
					listMap.values().forEach(function (d) { d.resetFilter(); });
				};
				...
				function addDimension(dim) {
					...
					var updateCallback = palladioService.onUpdate(domID, function() {
						actions.update();
					});

					var resetCallback = palladioService.onReset(domID, function() {
						actions.resetFilter();
					});

					deregister.push(updateCallback);
					deregisterFacet.push(updateCallback);

					deregister.push(resetCallback);
					deregisterFacet.push(resetCallback);

					actions.deregister = deregisterFacet;
