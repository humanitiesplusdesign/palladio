angular.module('palladioMapView', [])
	.constant('version', '0.9')
	.directive('palladioMapView', function () {
		...
		var directiveDefObj = {
			...
			link: function (scope, element, attrs) {
				...
				scope.$on('update', function(event) {
					// Only update if the map is visible.
				...
				scope.$watch('searchText', highlight);
			        ...
			        function draw() {
						...
						link
							...
							.on("click", function(d){
						    	...
						    	scope.$emit('updateFilter', [identifier, scope.title, scope.sourceAccessor(d.properties.data) + "/" + scope.destinationAccessor(d.properties.data), resetLink]);
								scope.$emit('triggerUpdate');
						    });
			            ...
			            node.exit().remove();
			          	...
			          	node
				          	...
				          	.on("click", function(d){
						    	...
						    	scope.$emit('updateFilter', [identifier, scope.title, d.properties.value.data.description, resetNode]);
								scope.$emit('triggerUpdate');
						    });
				...
				function highlight(){
		        	...
		        	node.classed("hidden", function(d){
		        		var found = d.properties.value.data.description.toLowerCase().indexOf(scope.searchText.toLowerCase()) !== -1;
		        		return found ? false : true;
		        	})

		        	link.classed("hidden", function(d){
		        		var found = sourceAccessor(d.properties.data).toLowerCase().indexOf(scope.searchText.toLowerCase()) !== -1 || destinationAccessor(d.properties.data).toLowerCase().indexOf(scope.searchText.toLowerCase()) !== -1;
		        		return found ? false : true;
		        	})
	...
	.directive('palladioMapViewWithSettings', function (dataService) {
		var directiveObj = {
			scope: true,
			...
			link: { pre: function(scope, element, attrs) {
				...
				scope.$emit('registerStateFunctions', ['mapView', exportState, importState]);
