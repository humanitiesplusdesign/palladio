...
angular.module('palladioTimelineFilter', ['palladio.date'])
	.constant('version', '0.9')
	.directive('palladioTimelineFilter', ['dateService', function (dateService) {
		...
		var directiveDefObj = {
			...
			link: function (scope, element, attrs) {
				...
				scope.$on('zoomToFilter', function(event) {
				...
				function setup() {
					...
					color = d3.scale.linear().interpolate(d3.interpolateLab).range(['#DDDDDD','#222222']);
	...
	.directive('palladioTimelineFilterWithSettings', ['dateService', function (dateService) {
		var directiveObj = {
			scope: true,
			...
			link: { pre: function(scope, element, attrs) {
					...
					scope.groupDims = scope.metadata
							.sort(function (a, b) { return a.description < b.description ? -1 : 1; });
					scope.groupProp = scope.groupDims[0] ? scope.groupDims[0] : undefined;
					...
					scope.$emit('registerStateFunctions', ['timeline', exportState, importState]);
	...
	.directive('palladioHistogramFilterWithSettings', function () {
		var directiveObj = {
			scope: true,
			template:'<div class="row-fluid">' +
						'<div class="span4">' +
							'<div class="accordion-heading">' +
								'<i class="icon-remove-sign close-button" data-ng-click="removeFilter($event)"></i>adsadas' +
						...
						'<div class="span8">' +
							'<div id="{{uniqueToggleId}}" class="row-fluid accordion-body collapse in component">' +
								...
								'<div class="span2 settings-panel"> ' +
									'<div class="row-fluid">' +
										'<div class="setting"> ' +
											'<label class="inline">Numeric dimension</label> ' +
											'<span class="field">{{numProp.description}}&nbsp;&nbsp;</span>' +
											'<button type="button" ng-click="showNumModal()" class="btn dim-selector"><i class="icon-list icon-white"></i></button>' +
										'</div> ' +
										'<div class="setting"> ' +
											'<label class="inline">Group by</label> ' +
											'<span class="field">{{groupProp.description}}&nbsp;&nbsp;</span>' +
											'<button type="button" ng-click="showGroupModal()" class="btn dim-selector"><i class="icon-list icon-white"></i></button>' +
										'</div> ' +
										'<div class="setting"> ' +
							...
							'</div>' +
							'<div id={{uniqueModalId}}>' +
								'<div id="num-modal" data-modal dimensions="numDims" model="numProp"></div>' +
								'<div id="group-modal" data-modal dimensions="groupDims" model="groupProp"></div>' +
							'</div>' +
						'</div>' + // chiusura span8
					'</div>',
			...
			link: { pre: function(scope, element, attrs) {
					...
					scope.numDims = scope.metadata.filter(function (d) { return d.type === 'number'; })
							.sort(function (a, b) { return a.description < b.description ? -1 : 1; });
					scope.numProp = scope.numDims[0] ? scope.numDims[0] : undefined;
					scope.$watch('numProp', function(nv, ov) {
						// Avoid switching the dateDim before the directive is prepared.
						if(nv !== ov) {
							if(scope.numDim) scope.numDim.remove();
							scope.numDim = scope.xfilter.dimension(function (d) { return +d[scope.numProp.key]; });
							scope.title = "Histogram: " + scope.numProp.description;
						}
					});
					...
					scope.title = scope.numProp ? "Histogram: " + scope.numProp.description : "Histogram";

					scope.numDim = scope.xfilter.dimension(function (d) { return +d[scope.numProp.key]; });
					scope.groupAccessor = function(d) { return d[scope.groupProp.key]; };

					scope.showNumModal = function () { $('#' + scope.uniqueModalId).find('#num-modal').modal('show'); };
					scope.showGroupModal = function () { $('#' + scope.uniqueModalId).find('#group-modal').modal('show'); };
					...
					// Move the modal out of the fixed area.
					$(element[0]).find('#num-modal').parent().appendTo('body');
				}
			}
		};
