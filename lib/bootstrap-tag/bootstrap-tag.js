...
!function ( $ ) {
  ...
  var Tag = function ( element, options ) {
    ...
    this.values = this.options.values || [];//$.grep($.map([this.element.val()], $.trim), function ( value ) { return value.length > 0 })
    this.show()
  }
  ...
  , add: function ( value ) {
      ...
      this.element.trigger('added', [this.values])
    }
  , remove: function ( index ) {
      if ( index >= 0 ) {
        ...
        this.element.trigger('removed', [this.values])
      }
    }
  , process: function () {
      var values = $.grep($.map([this.input.val()], $.trim), function ( value ) { return value.length > 0 }),
          that = this
